...
class DistilConfig(TrainingArguments):
    r"""
    Configuration class for the [`SelfDistillationTrainer`].
    ...
    Parameters:
        ...
        model_init_kwargs (`str`, `dict[str, Any]`, *optional*):
            ...
            argument of the [`SelfDistillationTrainer`] is provided as a string.
        ...
        sync_ref_model (`bool`, *optional*, defaults to `False`):
            ...
            the `ref_model_mixup_beta` parameter.
        ref_model_mixup_beta (`float`, *optional*, defaults to `0.4`):
            β parameter controlling the mixing between the previous reference policy and the current policy when
            the reference is refreshed. The reference policy is updated according to the equation:
            `π_ref = β * π_ref_prev + (1 - β) * π_θ`. Larger β keeps more of the previous reference (slower EMA),
            smaller β follows the student more closely. To use this parameter, you must set `sync_ref_model=True`.
    ...
    generate_from_teacher: bool = field(
        ...
        metadata={
            "help": "If True, use the teacher model (ref_model) for generation. vLLM will be initialized with teacher "
                   "weights, enabling fast generation from the teacher. This makes training equivalent to online "
    ...
    sync_ref_model: bool = field(
        ...
        metadata={
            ...
            "steps, using the `ref_model_mixup_beta` parameter."
        },
    )
    ref_model_mixup_beta: float = field(
        default=0.4,
        metadata={
            "help": "β parameter controlling the mixing between the previous reference policy and the current "
            "policy when the reference is refreshed. The reference policy is updated according to the equation: "
            "`π_ref = β * π_ref_prev + (1 - β) * π_θ`. Larger β keeps more of the previous reference (slower EMA), "
            "smaller β follows the student more closely. To use this parameter, you must set `sync_ref_model=True`."
        },
    )
