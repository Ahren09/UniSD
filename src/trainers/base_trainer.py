...
class MemoryEfficientSyncRefModelCallback(TrainerCallback):
    ...
    @staticmethod
    def _sync_param(model_param, ref_param, beta):
        """Sync a single parameter: ref = beta * ref + (1 - beta) * model"""
        ref_param.data.mul_(beta).add_(model_param.data, alpha=1.0 - beta)

    @staticmethod
    def sync_target_model_memory_efficient(model, target_model, beta):
        """
        Sync target_model to track model, gathering one parameter at a time.

        This is O(1) in tensor memory overhead instead of O(N) where N is model size.
        """
        def should_skip_param(name: str) -> bool:
            return any(
                s in name for s in ("lora_A", "lora_B", "lora_embedding_A", "lora_embedding_B")
            )
        ...
        # Canonicalized ref param lookup for PEFT-safe matching
        ref_params = {
            canonicalize_param_name(name): param
        ...
        if is_zero3:
            ...
            for name, model_param in model.named_parameters():
                ...
                key = canonicalize_param_name(name)
                ref_param = ref_params.get(key, None)

                if ref_param is not None and model_param.shape == ref_param.shape:
                    with deepspeed.zero.GatheredParameters(
                        [model_param, ref_param], modifier_rank=0
                    ):
                        if deepspeed.comm.get_rank() == 0:
                            MemoryEfficientSyncRefModelCallback._sync_param(
                                model_param, ref_param, beta
                            )
        else:
            for name, model_param in model.named_parameters():
                if should_skip_param(name):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if self.ref_model is not None and state.global_step % args.ref_model_sync_steps == 0:
            ...
            self.sync_target_model_memory_efficient(model, self.ref_model, args.ref_model_mixup_beta)
...
class SelfDistillationTrainer(BaseTrainer):
    """
    Base Trainer for Self-Distillation. 
    ```
    ...
    def __init__(
        ...
        if processing_class is None:
            processing_class = AutoProcessor.from_pretrained(model.config._name_or_path, truncation_side="left", trust_remote_code=True)
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in SelfDistillationTrainer. Please use a standard dataset instead."
            )
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In SelfDistillationTrainer, we preprocess data, so using the model's signature columns doesn't work.
        # Instead, we set them to the columns expected by the `training_step` method, hence the override.
        if self._signature_columns is None:
            self._signature_columns = ["prompt", "teacher_prompt", "bad_teacher_prompt", "image", "images"]
    ...
    def _get_per_token_logps_and_entropies(
        self,
        model,
        input_ids, # This is usually concatenation of prompt and completion
        attention_mask,
        logits_to_keep,
        batch_size=None,
        ...
        for start in range(0, input_ids.size(0), batch_size):
            ...
            logits = logits / (logit_temperature if logit_temperature is not None else self.temperature)
            ...
            if return_hidden_states:
                last_hs = model_outputs.hidden_states[-1]
                last_hs = last_hs[:, :-1, :]
                last_hs = last_hs[:, -logits_to_keep:, :]
        ...
        return selected_logps, logps, entropies, hidden_states
    ...
    def _generate_and_score_completions(
        ...
        with torch.no_grad():
            ...
            if not self.generate_from_teacher and (
                ...
                old_per_token_logps, _, _, _ = self._get_per_token_logps_and_entropies(
                    self.model,
            ...
            if self.beta != 0.0:
                if self.ref_model is not None:
                    ref_per_token_logps, _, _, _ = self._get_per_token_logps_and_entropies(
                        self.ref_model,
                        prompt_completion_ids,
                        attention_mask,
                        logits_to_keep,
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The SelfDistillationTrainer does not support returning outputs")
    ...
    def _compute_loss(self, model, inputs):
        ...
        per_token_logps, all_logps, entropies, _ = self._get_per_token_logps_and_entropies(
            model,
            input_ids,
            attention_mask,
            logits_to_keep,
        ...
        with torch.no_grad():
            if self.ref_model is not None:
                teacher_model_for_logps = self.ref_model
                teacher_ctx = nullcontext()
            else:
                teacher_model_for_logps = model
                teacher_ctx = self.accelerator.unwrap_model(model).disable_adapter()
            with teacher_ctx:
                teacher_per_token_logps, teacher_all_logps, teacher_entropies, _ = self._get_per_token_logps_and_entropies(
                    teacher_model_for_logps,
                    teacher_input_ids,
